...
> Engine undangan digital: 101 tema, 18,6 KB engine gzip, 1 JSON per klien.
...
> **Untuk WO/MUA yang mau jual undangan digital pakai brand sendiri, Ikat adalah engine undangan white-label yang halaman pertamanya 22 KB — kebuka di sinyal satu bar — dengan 101 tema siap jual dan margin 80% per undangan.**

Untuk pasangan langsung:

> **Undangan yang kebuka, bukan yang bikin tamu nunggu. 18,6 KB, 101 tema, ganti tema tanpa bikin ulang.**
...
| **Plus** | Rp 199.000 | 101 tema gonta-ganti, warna custom, galeri, musik custom, amplop digital, rekap unduh | 2 tahun | B2C mainstream — **hero product** |
...
| **Hook yang terbukti** | "Undangan 22 KB vs 5 MB — tes di sinyal 1 bar" (split screen), "Nggak punya foto prewed? Coba ini" (before/after monogram), "Ganti 101 tema dalam 10 detik" | 3 hook, rotasi |
| **Hashtag** | #UndanganDigital #UndanganPernikahan #UndanganOnline #WeddingTikTok #TanpaPrewed | 5 hashtag/video |
| **CTA** | "Link di bio — coba 101 tema gratis" (ke preview.html) | Klik bio → preview |
| **Programmatic video (Remotion)** | Generate varian video otomatis dari React — lihat §5.3. 101 tema × 3 hook × 2 format (9:16, 1:1) = 606 varian tanpa edit manual. Render via `npx remotion render` atau `@remotion/renderer` self-host. | Skala konten tanpa editor |
...
- 101 pin — 1 per tema (foto sampul + link preview). Pinterest adalah search engine visual — umur pin 6–12 bulan vs 24 jam di IG.
...
| **Instagram Ads (Reels placement)** | Rp 30.000/hari (Rp 900rb/bulan) | Lookalike dari follower + interest WO/MUA | Carousel 101 tema | CPC < Rp 1.200 |
...
| **Tema & Estetika** | 30% | "101 tema Ikat — ganti dalam 10 detik", "Warna 2026: Butter Yellow vs Cloud Dancer" | Video showcase, Pinterest pin |
...
| Rabu | TikTok + Reels | Showcase 1 tema (rotasi 101 tema = 101 hari showcase tanpa ulang) |
...
**Peluang untuk Ikat:** Ikat sudah punya 101 tema sebagai data JSON (`themes/themes.json`). Remotion bisa generate video marketing massal tanpa editor:

| Use case | Cara | Output |
|---|---|---|
| **Showcase 101 tema** | 1 komponen Remotion baca `themes.json` → render tiap tema sebagai card animasi (fade + spring) | 101 video 15 detik (1 per tema) + 1 kompilasi 60 detik |
| **Varian hook** | `inputProps` = `{ theme, hook, cta }` → parameterized rendering | 101 tema × 3 hook = 303 varian otomatis |
...
# Batch: loop 11 tema
for t in $(python3 -c "import json; print(' '.join(t['slug'] for t in json.load(open('themes/themes.json'))['themes']))"); do  # 101 tema
...
- **CTA selalu 1:** "Coba 101 tema gratis — link di bio" (jangan "follow + like + komen + share" sekaligus).
...
| 1 | Buat 3 hook video (22 KB test, ganti 101 tema, tanpa prewed) — rekam layar + CapCut, 9:16 | 3 video jadi | Solo |
...
| 3 | Setup Pinterest: 101 pin (1 per tema) | 101 pin live | Solo |
...
| 6 | Automasi video (jika 1 hook > 10rb views): setup Remotion — 1 komponen baca `themes.json`, generate 101 varian showcase | 11 video Remotion jadi |
...
| 9 | Scale Remotion: generate 33 varian (101 tema × 3 hook) untuk TikTok Ads A/B test | 33 video varian |
...
## Appendix: Tooling — aset, video, dan workflow
